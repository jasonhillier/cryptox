        ...
        poloniexPrivate.myAvailableAccountBalances(function (err, xResult) {
            ...
            if (!err)
            {
                for(var account in xResult)
                {
                    ...
                    if (account === 'exchange')
                    {
                        //create a reference to the exchange account for later use
                        tmpExchangeAccount = tmpAccount;
                    }
            ...
            poloniexPrivate.myCompleteBalances(function(err, xResult)
            {
                tmpExchangeAccount.total = [];
                for(var currency in xResult)
                {
                    var tmpCurrencyTotalBalance = {currency: currency};
                    tmpCurrencyTotalBalance.amount = xResult[currency].available*1 + xResult[currency].onOrders*1;
                    
                    if (tmpCurrencyTotalBalance.amount > 0)
                    {
                        tmpExchangeAccount.total.push(tmpCurrencyTotalBalance);
                    }
                }
